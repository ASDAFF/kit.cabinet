...
if(Loader::includeModule('kit.cabinet') && $idUser > 0)
{
	...
	<div class="gdsetcartinfo">
		<span>
			<?= $Items->getQnt() ?> <?= \Kit\Cabinet\Element::num2word(
				$Items->getQnt(),
				array(
						Loc::getMessage('GD_KIT_CABINET_DELAYBASKET_PRODUCTS_1'),
						Loc::getMessage('GD_KIT_CABINET_DELAYBASKET_PRODUCTS_2'),
						Loc::getMessage('GD_KIT_CABINET_DELAYBASKET_PRODUCTS_3')
				));
			?>
		</span>
	</div>
	...
	if($arParams['G_DELAYBASKET_PATH_TO_BASKET'])
	{
		...
		<div class="gdhtmlareachlink">
			<a href="<?= $arParams['G_DELAYBASKET_PATH_TO_BASKET'] ?>">
				<?= Loc::getMessage('GD_KIT_CABINET_DELAYBASKET_MORE') ?>
			</a>
		</div>
		<?
	}
